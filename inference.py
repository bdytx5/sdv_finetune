...
from diffusers import AutoencoderKLTemporalDecoder, UNetSpatioTemporalConditionModel, StableVideoDiffusionPipeline
from diffusers import AutoencoderKLTemporalDecoder, EulerDiscreteScheduler, UNetSpatioTemporalConditionModel
...
def run_inference(model_name_or_path, image_path, output_path, device="cuda"):
    # Initialize device
    device = torch.device(device if torch.cuda.is_available() else "cpu")
    
    # Load pre-trained models
    noise_scheduler = EulerDiscreteScheduler.from_pretrained(model_name_or_path, subfolder="scheduler")
    feature_extractor = CLIPImageProcessor.from_pretrained(model_name_or_path, subfolder="feature_extractor")
    image_encoder = CLIPVisionModelWithProjection.from_pretrained(model_name_or_path, subfolder="image_encoder")
    vae = AutoencoderKLTemporalDecoder.from_pretrained(model_name_or_path, subfolder="vae")
    unet = UNetSpatioTemporalConditionModel.from_pretrained(model_name_or_path, subfolder="unet")
    ...
    input_image = load_image(image_path).resize((1024, 576))
    
    # Initialize the diffusion pipeline
    pipeline = StableVideoDiffusionPipeline(unet=unet, image_encoder=image_encoder, vae=vae, feature_extractor=feature_extractor, noise_scheduler=noise_scheduler).to(device)
    
    # Generate video frames
    video_frames = pipeline(input_image, height=576, width=1024, num_frames=25, guidance_scale=7.5).frames
    
    # Save the generated frames as a video or GIF
    export_to_video(video_frames, output_path, fps=30)  # Assuming export_to_video is defined

# Example usage
model_name_or_path = "stabilityai/stable-video-diffusion-img2vid-xt"
image_path = "path/to/your/input/image.jpg"
output_path = "path/to/save/generated/video.mp4"
run_inference(model_name_or_path, image_path, output_path)
