                ...
                if accelerator.is_main_process:
                    ...
                    if (
                        ...
                        with torch.autocast(
                            ...
                            # val_image_paths = glob.glob(os.path.join(args.val_dir, '*.jpg'))  # Adjust the pattern as needed for your image file types

                            # for val_img_idx, img_path in enumerate(val_image_paths):
                            #     # Load the image
                            #     img = Image.open(img_path).resize((args.width, args.height))
                            #     img_array = np.array(img)
                                
                            #     # If additional processing is required, do it here
                                
                            #     # Save or process the image as needed
                            #     # For example, saving each image as a GIF (though normally you'd use GIFs for sequences)
                            #     out_file = os.path.join(
                            #         val_save_dir,
                            #         f"val_img_{val_img_idx}.gif",  # Naming the output file
                            #     )
                                
                            #     # Assuming export_to_gif can handle single images, otherwise save it in another format
                            #     export_to_gif([img_array], out_file, fps=8)  # Adjust as necessary if your function expects different parameters
...
# import numpy as np
                            ...
                            # Assuming definitions for args, pipeline, load_image, export_to_gif and other necessary imports and global variables are done above

                            val_image_paths = glob.glob(os.path.join(args.val_dir, '*.jpg'))  # Adjust the pattern as needed for your image file types
                            val_save_dir = './'  # Make sure this is defined or replace with your directory path

                            for val_img_idx, img_path in enumerate(val_image_paths):
                                num_frames = args.num_frames
                                video_frames = pipeline(
                                    load_image(img_path).resize((args.width, args.height)),
                                    height=args.height,
                                    width=args.width,
                                    num_frames=num_frames,
                                    decode_chunk_size=8,
                                    ...
                                    fps=7,
                                    noise_aug_strength=0.02,
                                    # generator=generator,  # Uncomment and define generator if needed
                                ).frames[0]

                                out_file = os.path.join(
                                    val_save_dir,
                                    f"step_{global_step}_val_img_{val_img_idx}.mp4",  # Assuming global_step is defined somewhere
                                )

                                for i in range(num_frames):
                                    img = video_frames[i]
                                    video_frames[i] = np.array(img)
                                export_to_gif(video_frames, out_file, fps=8)
